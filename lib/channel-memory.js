...
export class ChannelMemory {
	...
	scheduleAsyncCompression(entryId) {
		...
		this.runAsyncCompression(entryId)
			.catch(() => {
	...
	async runAsyncCompression(entryId) {
		...
		for (let attempt = 0; attempt < this.compressionRetries; attempt++) {
			try {
				const session = await Promise.race([this.getSession(), this.timeout(this.compressionTimeout, "Session acquire")]);
	...
	reconstructEntries(entry) {
		...
		const matchingRecent = this.state.recent.filter((e) => entry.scenes.includes(e.scene));
	...
	async compressWithSession(session, entries) {
		const turns = entries.flatMap((e) => (e.turns ?? []).map((t) => `${t.speaker}: ${t.text}`));

...
export async function readDaemonStatus(paths) {
	const [pidFilePid, lockState, status] = await Promise.all([readNumericFile(paths.pidPath), readJsonFile(paths.lockPath), readJsonFile(paths.statusPath)]);
...
export async function startDaemon(paths) {
	...
	if (state.running) {
		return {
			started: false,
			reason: `Daemon already running as pid ${state.pid}.`,
		};
	}

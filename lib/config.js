	...
	for (const [agentName, agent] of Object.entries(value)) {
		...
		normalized[agentName] = {
			...
			defaultThinkingLevel:
				typeof agent.defaultThinkingLevel === "string" && THINKING_LEVELS.has(agent.defaultThinkingLevel) ? agent.defaultThinkingLevel : undefined,
		};
	}
	return normalized;
}
...
function normalizeSliceOfBread(value) {
	if (!value || typeof value !== "object" || Array.isArray(value)) {
	...
	return {
		...
		boardChannelId: typeof value.boardChannelId === "string" ? value.boardChannelId : "",
		...
		memoryPath: typeof value.memoryPath === "string" ? value.memoryPath : undefined,
		memoryMaxTokens: typeof value.memoryMaxTokens === "number" ? value.memoryMaxTokens : 8192,
		checkInterval: typeof value.checkInterval === "number" ? value.checkInterval : 60000,
		topicMemorySize: typeof value.topicMemorySize === "number" ? value.topicMemorySize : 10,
		globalTopics: toStringArray(value.globalTopics),
		generateTopics: Boolean(value.generateTopics),
		searchEnabled: value.searchEnabled !== false,
		botFollowup: value.botFollowup ?? undefined,
		worldNews: value.worldNews ?? undefined,
		scenes: Array.isArray(value.scenes) ? value.scenes.map(normalizeScene) : [],
	};
}

function normalizeRouteCleanup(value) {
	if (!value || typeof value !== "object" || Array.isArray(value)) {
		return { enabled: false, maxAgeDays: 30, onStartup: true };
	}
	return {
		enabled: Boolean(value.enabled),
		maxAgeDays: typeof value.maxAgeDays === "number" ? value.maxAgeDays : 30,
 ...
 * @property {{ lines?: number, chars?: number } | undefined} [contextLimits]
 */
 ...
 * @property {boolean} enableChannelMemory
 ...
 * @property {SliceOfBreadConfig} sliceOfBread
 */
...
export function normalizeConfig(paths, loaded) {
	...
	return {
		...
		enableChannelMemory: typeof input.enableChannelMemory === "boolean" ? input.enableChannelMemory : fallback.enableChannelMemory,
		...
		defaultThinkingLevel:
			typeof input.defaultThinkingLevel === "string" && THINKING_LEVELS.has(input.defaultThinkingLevel)
		...
		systemPromptFile: normalizeOptionalString(input.systemPromptFile) ?? fallback.systemPromptFile,
		useThreadPersona: typeof input.useThreadPersona === "boolean" ? input.useThreadPersona : fallback.useThreadPersona,
		showThinkingStatus: typeof input.showThinkingStatus === "boolean" ? input.showThinkingStatus : fallback.showThinkingStatus,
		toolPermissions: normalizeToolPermissions(input.toolPermissions),
		agents: normalizeAgents(input.agents),
		defaultAgent: normalizeOptionalString(input.defaultAgent) ?? fallback.defaultAgent,
		sliceOfBread: normalizeSliceOfBread(input.sliceOfBread),
		routeCleanup: normalizeRouteCleanup(input.routeCleanup),
		presence: input.presence ?? undefined,
		botFollowup: input.botFollowup ?? undefined,
	};
}

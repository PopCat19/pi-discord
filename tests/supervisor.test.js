...
test("readDaemonStatus falls back to live lock or status pids when daemon.pid is stale", async () => {
	const tempDir = await mkdtemp(
		path.join(os.tmpdir(), "pi-discord-supervisor-"),
	);
	...
	await writeFile(
		paths.statusPath,
		JSON.stringify({ pid: process.pid, phase: "running" }),
		"utf8",
	);
...
test("readDaemonStatus drops stale status payloads from dead pids", async () => {
	const tempDir = await mkdtemp(
		path.join(os.tmpdir(), "pi-discord-supervisor-stale-"),
	);
	const paths = getPaths({
		agentDir: tempDir,
		workspaceDir: path.join(tempDir, "workspace"),
	...
	await writeFile(
		paths.statusPath,
		JSON.stringify({ pid: 999999, phase: "stale" }),
		"utf8",
	);
...
test("readDaemonStatus does not trust status payloads without a matching live pid", async () => {
	const tempDir = await mkdtemp(
		path.join(os.tmpdir(), "pi-discord-supervisor-missing-pid-"),
	);
	const paths = getPaths({
		agentDir: tempDir,
		workspaceDir: path.join(tempDir, "workspace"),
	...
	await writeFile(
		paths.statusPath,
		JSON.stringify({ phase: "running-without-pid" }),
		"utf8",
	);
...
test("readDaemonStatus does not report stale status for dead daemons", async () => {
	const tempDir = await mkdtemp(
		path.join(os.tmpdir(), "pi-discord-supervisor-dead-"),
	);
	const paths = getPaths({
		agentDir: tempDir,
		workspaceDir: path.join(tempDir, "workspace"),
	...
	await writeFile(
		paths.statusPath,
		JSON.stringify({ pid: 999999, phase: "old" }),
		"utf8",
	);
